...
function Go_to_level (extra: boolean) {
    if (Level == 1) {
        ...
        if (extra) {
            for (let 值 of V_list) {
                值.setFlag(SpriteFlag.Invisible, true)
            }
            for (let 值2 of H_list) {
...
for (let 值3 of V_list) {
    值3.setFlag(SpriteFlag.StayInScreen, true)
    值3.setFlag(SpriteFlag.Invisible, true)
}
for (let 值4 of H_list) {
